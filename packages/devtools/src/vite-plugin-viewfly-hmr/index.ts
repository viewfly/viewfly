...
import { RENDERER_HMR_REGISTER_REPLACEMENT } from './renderer-hmr-inject'
 ...
 * 开发服务下会将 `optimizeDeps.exclude` 设为包含 `@viewfly/core`，并对其中
 * `__viewflyHmrRegisterSlotMarker()` 锚点做 transform，注入对 `globalThis.__VF_HMR__` 的注册（与 `hmr-runtime/install` 配合）。
...
export function viewflyHmr(options: ViewflyHmrOptions = {}): VitePlugin {
  ...
  return {
    name: 'viewfly-hmr',
    enforce: 'pre',
    config(_cfg, env): UserConfig {
      return {
        esbuild: {
          jsx: 'preserve',
        },
        /** 开发服务下便于对 `@viewfly/core` 做 transform 注入，避免预打包成单 chunk 后丢失替换锚点 */
        optimizeDeps:
          env.command === 'serve'
            ? {
                exclude: ['@viewfly/core'],
              }
            : undefined,
      } as UserConfig
    },
    ...
    transform(code, id) {
      if (
        isServe
        && isDev
        && code.includes('__viewflyHmrRegisterSlotMarker(')
      ) {
        const injected = code.replace(
          /\b__viewflyHmrRegisterSlotMarker\s*\(\s*\)/,
          RENDERER_HMR_REGISTER_REPLACEMENT,
        )
        if (injected !== code) {
