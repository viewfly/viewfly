...
import { rerunComponentSetup } from '@viewfly/core'
import { installHotJsxCreateNodePatch } from './jsx-create-node-patch'
...
/**
 * 安装 HMR：包装 `JSXNodeFactory.createNode` 以登记热边界实例；setup 重跑由 `@viewfly/core` 的
 * `rerunComponentSetup` 完成（由 `createRenderer` 注册处理器）。
 *
 * 须在首屏组件挂载前调用；通常由 `@viewfly/devtools/vite-plugin-viewfly-hmr` 注入 install 子路径。
 */
export function installViewflyHmr(): void {
  installHotJsxCreateNodePatch((id) => getCache().get(id))
}
