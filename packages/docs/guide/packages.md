...
| [@viewfly/devtools](https://www.npmjs.com/package/@viewfly/devtools) | 构建侧：`*.scoped.*` 样式与 `Vite` / `Rollup` / `Webpack` 集成。 |
| [@viewfly/cli](https://www.npmjs.com/package/@viewfly/cli) | 脚手架，生成 `Vite` + `TypeScript` 模板。 |

路由与 scoped CSS 均为可选；scoped CSS 需 **`@viewfly/core`**（如 `withMark`）与 **`@viewfly/devtools`** 配合使用。
