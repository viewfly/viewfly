...
function getOrAddVfMakeHot(program: NodePath<t.Program>, state: State): t.Identifier {
  if (state.vfMakeHotId) {
    return state.vfMakeHotId
  }
  state.vfHotUsed = true
  const id = addNamed(program, '__vfMakeHot', '@viewfly/devtools/hmr-runtime', { nameHint: '__vfMakeHot' })
  state.vfMakeHotId = id
  return id
}
...
function wrapExportDefaultFunction(
  ...
  const vf = getOrAddVfMakeHot(program, state)
  ...
  const defaultId = path.scope.generateUidIdentifier('vfDefaultExport')
  const hot = t.variableDeclaration('const', [
  ...
  path.replaceWithMultiple([fnDecl, hot, t.exportDefaultDeclaration(defaultId)])
}
...
function wrapExportNamedFunction(
  ...
  path.replaceWithMultiple([fnDecl, hot, reExport])
}
...
function wrapExportNamedConstFunction(
  ...
  path.replaceWithMultiple([implDecl, hot, reExport])
}
